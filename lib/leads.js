...
function resolveTeamAbbrev(teamStr, pools) {
  ...
  // two passes: full-string containment first, nickname (last word) only as a
  // fallback — else "Chicago White Sox" hits Boston via the shared "Sox"
  const hit = (pools || []).find((p) => {
    const n = (p.teamName || "").toLowerCase();
    return n && (n.includes(low) || low.includes(n));
  }) || (pools || []).find((p) => {
    const n = (p.teamName || "").toLowerCase();
    return n && low.includes(n.split(" ").pop());
  });
