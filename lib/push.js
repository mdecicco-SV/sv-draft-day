...
async function notifyPick(redis, p, { sentKey, titlePrefix = "", tagPrefix = "sv-pick" } = {}) {
  ...
  // iOS inserts a "from <PWA name>" attribution line between title and body on
  // every web push — it can't be removed, so the player's name must live in the
  // title (above it) or the attribution obscures him.
  return sendToAll(redis, {
    title: `${titlePrefix}${p.player}`,
    body: `SV Client Drafted\n${p.teamName || p.team || "?"} (#${p.overall}) — Rd ${p.round}`,
